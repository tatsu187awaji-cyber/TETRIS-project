...
function applyPostClear(arena, gameState, linesCleared, tSpinType) {
  const isSpecial =
    linesCleared === 4 ||
    (tSpinType && linesCleared >= 0 && tSpinType === "full") ||
    (tSpinType === "mini" && linesCleared > 0);
  ...
  const isPerfectClear =
    linesCleared > 0 && arena.every((row) => row.every((cell) => cell === 0));

  // フラッシュメッセージ
  if (
    !gameState.flashMessage ||
    !gameState.flashMessage.text.includes("LEVEL UP")
  ) {
    if (isPerfectClear) {
      ...
      gameState.flashMessage = {
        text: wasB2B && isSpecial ? "B2B T-SPIN!" : "T-SPIN",
        timer: 120,
        color: "#ff4400",
      };
      ...
      gameState.flashMessage = {
        text: wasB2B && isSpecial ? "B2B TETRIS!" : "TETRIS!",
        timer: 120,
        color: "#00eeff",
      };
    } else if (gameState.combo > 1) {
      gameState.flashMessage = {
        text: `${gameState.combo} REN!`,
        timer: 120,
        color: "#ffff00",
      };
    }
...
export function handlePlayerDrop(
  ...
  if (collide(arena, player)) {
    ...
    const isGameOver = playerReset(arena, pieceBag, gameState); // ← 戻り値を受け取る
    updateScore();
    return isGameOver; // ← true はゲームオーバー時だけ
  }
...
export function handleHardDrop(arena, gameState, pieceBag, updateScore) {
  ...
  triggerPlacementFlash(gameState, player);
  console.log("merge前 player.pos:", player.pos.x, player.pos.y);
  merge(arena, player);
